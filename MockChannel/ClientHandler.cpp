...
ClientHandler::ClientHandler(Recvr* rev)
{
	myReceiver = rev;
}
	...
	try
	{
		while (true)
		{
			...
			else if (msg.getMessage()["commandType"] == "File_Download")
			{
				...
				{
					msg.setFilePath("File Downloaded Successfully at /DownloadedFiles");
					myReceiver->enqueue(msg);
				}
				else
			...
			{
				Verbose::show("In File Search");
				std::string t = "File Search Results are\n";
				t += msg.getMessage()["filePath"];
				msg.setFilePath(t);
				myReceiver->enqueue(msg);
			}
			else if (msg.getMessage()["commandType"] == "Text_Search")
			{
				Verbose::show("In Text Search");
				std::string t = "Text Search Results are\n";
				t += msg.getMessage()["filePath"];
				msg.setFilePath(t);
				myReceiver->enqueue(msg);
			}	
...
{
	std::string path = "../CppCli-WPF-App/DownloadedFiles/";
